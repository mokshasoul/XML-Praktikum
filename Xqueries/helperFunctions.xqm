...
let $pattern := doc('sampleCalendarX2.xml')//patterns/*[@description=$patternName]
...
return switch($pattern/name())
 ...
 case "cardinalMonthlyPattern" return $pattern/@dayOfMonth eq fn:day-from-date($date)
 case "ordinalMonthlyPattern" return helper:getOrdinalOfMonth($date, $pattern/@ordinal, $pattern/@dayType) eq $date
 case "cardinalYealyPattern" return $pattern/@dayOfMonth eq fn:day-from-date($date) and $pattern/@month eq functx:month-name-en($date)
  case "ordinalYearlyPattern" return helper:getOrdinalOfMonth($date, $pattern/@ordinal, $pattern/@dayType) eq $date  and $pattern/@month eq functx:month-name-en($date) 
...
declare function helper:getOrdinalOfMonth($date as xs:date, $ordinal, $dayType){
    ...
    let $firstDayOfType := switch($dayType) 
    case "day"  return $firstOfMonth
    case "weekDay" return if (functx:day-of-week-name-en($firstOfMonth) eq "Saturday") then $firstOfMonth+xs:dayTimeDuration('P2D') else if (functx:day-of-week-name-en($firstOfMonth) eq "Sunday") then $firstOfMonth+xs:dayTimeDuration('P1D') else $firstOfMonth
    case "Monday" return $firstOfMonth+xs:dayTimeDuration(concat('P',6-((functx:day-of-week($firstOfMonth)+5) mod 7),'D'))
    case "Tuesday" return $firstOfMonth+xs:dayTimeDuration(concat('P',6-((functx:day-of-week($firstOfMonth)+4) mod 7),'D'))
    case "Wednesday" return $firstOfMonth+xs:dayTimeDuration(concat('P',6-((functx:day-of-week($firstOfMonth)+3) mod 7),'D'))
    case "Thursday" return $firstOfMonth+xs:dayTimeDuration(concat('P',6-((functx:day-of-week($firstOfMonth)+2) mod 7),'D'))
    case "Friday" return $firstOfMonth+xs:dayTimeDuration(concat('P',6-((functx:day-of-week($firstOfMonth)+1) mod 7),'D'))
    case "Saturday" return $firstOfMonth+xs:dayTimeDuration(concat('P',6-((functx:day-of-week($firstOfMonth)+0) mod 7),'D'))
    case "Sunday" return $firstOfMonth+xs:dayTimeDuration(concat('P',6-((functx:day-of-week($firstOfMonth)+6) mod 7),'D'))
    default return $firstOfMonth
    ...
    let $ultimateDayOfType := if(fn:month-from-date($ultimateDayOfTypeSuggestion) eq fn:month-from-date($firstDayOfType)) then $ultimateDayOfTypeSuggestion else $ultimateDayOfTypeSuggestion - xs:dayTimeDuration('P7D')
    let $penultimateDayOfType := $ultimateDayOfType - xs:dayTimeDuration('P7D')
    
    return switch($ordinal)
        case "first" return $firstDayOfType
        case "second" return ($firstDayOfType + xs:dayTimeDuration('P7D'))
        case "third" return ($firstDayOfType + xs:dayTimeDuration('P14D'))
        case "fourth" return ($firstDayOfType + xs:dayTimeDuration('P21D'))
        case "penultimate" return $penultimateDayOfType
        case "ultimate" return $ultimateDayOfType
    default return $firstDayOfType
};
