    ...
    <xsl:variable name="displacement">
        <xsl:choose>
            <xsl:when test="$packedView eq 'true'">
                <xsl:value-of select="150+100*events/@maxNumOfEventsOnDay"/>
            </xsl:when>
            <xsl:otherwise>
                <xsl:value-of select="1544"/>
            </xsl:otherwise> <!--1440 (timeline) +100 (header) +4 (horizontal line)-->
        </xsl:choose>
    </xsl:variable>
    <xsl:variable name="scale" select="0.35"/>
    <xsl:template match="/">
        <svg xmlns="http://www.w3.org/2000/svg" height="{if ($mode != 'Month') then 1544 else $displacement*$scale*$weeksInThisMonth/maxWeek}" width="100%">
            <defs>
                ...
                <xsl:for-each select="events/event">
                    <xsl:call-template name="foo:generateBasicTask">
                        <xsl:with-param name="event">
                            <xsl:value-of select="."/>
                        </xsl:with-param>
                        <xsl:with-param name="packedView">
                            <xsl:value-of select="$packedView"/>
                        </xsl:with-param>
                    </xsl:call-template>
                </xsl:for-each>
                    ...
                    <g id="T{@date}">
						...
						-->
                            <xsl:for-each select="current-group()">
                                <xsl:call-template name="foo:translateTaskForDayView">
                                    <xsl:with-param name="event">
                                        <xsl:value-of select="."/>
                                    </xsl:with-param>
                                    <xsl:with-param name="packedView">
            ...
            <xsl:choose>
                <xsl:when test="$mode eq 'Day' ">
					<!--DISPLAY DAY-->
                    <g transform="scale(0.4,0.4)">
                        <xsl:if test="$packedView eq 'false'">
                ...
                <xsl:when test="$mode eq 'Week' ">
					<!--DISPLAY WEEK-->
                    <g transform="scale(0.35,0.35)">
                        <xsl:if test="$packedView eq 'false'">
                            <use xlink:href="#TIMES" transform="translate(0,15)"/>
                        </xsl:if>
