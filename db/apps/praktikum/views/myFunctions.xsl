...
returns the scaling factor for scaling the event box accoridng to the duratio of the event-->
    ...
    <xsl:template xmlns="http://www.w3.org/2000/svg" name="foo:printVerticalLine">
        <g id="VLINE">
            <g transform="translate(-2,0)">
                <line x1="0" y1="0" x2="0" y2="1560" style="stroke:#7125FE;stroke-width:4"/>
            </g>
        </g>
    </xsl:template>
    <xsl:template xmlns="http://www.w3.org/2000/svg" name="foo:printOverlay">
        <rect id="overlayX" width="500" height="130" style="fill:#BEBECB"/>
    </xsl:template>
    <xsl:template xmlns="http://www.w3.org/2000/svg" name="foo:printHorizontalLine">
        <g id="HLINE">
            <line x1="20" y1="0" x2="3545" y2="0" style="stroke:#7125FE;stroke-width:4"/>
        </g>
    </xsl:template>
    ...
    <xsl:function name="foo:getCertainDayOfWeek">
		...
		<!-- 
		 ...
		 -->
    <xsl:template xmlns="http://www.w3.org/2000/svg" name="foo:printWeekOfDay">
        <xsl:param name="date"/>
        <xsl:variable name="mondayOfWeek">
    ...
    <xsl:template xmlns="http://www.w3.org/2000/svg" name="foo:printEventDescription">
        ...
        <text transform="translate(0,15)" font-size="30">
            <tspan x="10" y="10" font-size="20">
                <xsl:value-of select="fn:format-time(xs:time($startTime), '[H01]:[m01]')"/>-<xsl:value-of select="fn:format-time(xs:time($endTime), '[H01]:[m01]')"/> |  <xsl:value-of select="location/@description"/>
            </tspan>
...
<!--GENERATE BASIC TASK-->
    <xsl:template xmlns="http://www.w3.org/2000/svg" name="foo:generateBasicTask">
        <xsl:param name="event"/>
        <xsl:param name="packedView"/>
        <xsl:variable name="scaleFactorX">
            <xsl:choose>
                <xsl:when test="$packedView eq 'true' or @intersecting=0">
        ...
        <xsl:variable name="scaleFactorY">
            <xsl:choose>
                <xsl:when test="$packedView eq 'true'">
                    <xsl:value-of select="1"/>
                </xsl:when>
                <xsl:otherwise>
        ...
        <g id="T{@id}">
            <g transform="scale({$scaleFactorX},{$scaleFactorY})">
                <use xlink:href="#APP"/>
            </g>
	...
	<!--TRANSLATE TASK FOR DAY VIEW-->
    <xsl:template xmlns="http://www.w3.org/2000/svg" name="foo:translateTaskForDayView">
        <xsl:param name="event"/>
        <xsl:param name="packedView"/>
        <xsl:variable name="displacementY">
            <xsl:choose>
                <xsl:when test="$packedView eq 'true'">
                    <xsl:value-of select="100*(position()-1)"/>
                </xsl:when>
                <xsl:otherwise>
                    <xsl:value-of select="foo:getTimeInMinutes(@startTime)"/>
                </xsl:otherwise>
            </xsl:choose>
        </xsl:variable>
        <xsl:variable name="displacementX">
            <xsl:choose>
                ...
                <xsl:otherwise>
                    <xsl:value-of select="@pos*(500*0.96 div (@intersecting+1))"/>
                </xsl:otherwise>
            </xsl:choose>
        </xsl:variable>
        <g transform="translate({$displacementX},{$displacementY})">
            <use xlink:href="#T{@id}" transform="scale(0.96,1)"/>
        </g>
    </xsl:template>
</xsl:stylesheet>